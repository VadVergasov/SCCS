...
class ColorConverterMiddleware {
    static toRgb(color) {
        if (typeof color === 'string' || color instanceof String) {
            ...
            if (color.length !== 3) {
                throw new Error("Incorrect array to convert: lenght must be three.");
            }
    ...
    static toHex(color) {
        ...
        // (1 << 24) with slice neded for leading zeros 
    ...
    static sumColors(color1, color2) {
        ...
        return this.toHex(rgb1);
    }

    static subtractColors(color1, color2) {
        let rgb1 = this.toRgb(color1);
        let rgb2 = this.toRgb(color2);
...
export default ColorConverterMiddleware;